...
sealed interface LaunchAuthorizationState {
     ...
     */
    data class Pending(val createdAtMs: Long) : LaunchAuthorizationState

    /** 已激活：豆包在前台，守卫放行。 */
    data class Active(val activatedAtMs: Long) : LaunchAuthorizationState

    /**
     * 暂停（M0.5 预留）：计时阶段豆包离开前台时授权暂停、计时挂起。
     * M0.4 的单次会话语义不产生此状态（离开前台直接失效）。
     */
...
/**
 * M0.4 临时授权状态机（纯逻辑，时钟由调用方传入，保证纯 JVM 可测）。
 *
 * 转换规则（单次会话）：
 * - Idle --createPending--> Pending：仅守卫入口可触发，Pending/Active 期间拒绝重复创建；
 * - Pending --豆包前台(未超时)--> Active：放行，豆包正式启动；
...
class LaunchAuthorizationStateMachine {
     ...
     *
     * Pending 未超时 -> 激活为 Active 并放行；Active -> 放行；
     * Pending 已超时 -> 惰性转 Revoked 并拒绝；Idle/Revoked/Paused -> 拒绝。
     */
        ...
        return when (val current = state) {
            is LaunchAuthorizationState.Pending -> {
                if (nowMs - current.createdAtMs > PENDING_VALIDITY_MS) {
                    ...
                    state = LaunchAuthorizationState.Active(nowMs)
                    true
                }
            }

            is LaunchAuthorizationState.Active -> true
